...
;(function (window, undefined) {
  ...
  function dateEvent() {
    ...
    var dataTime = null
    var popupPos;
    if(detail_festival == null || detail_festival.length==0){
    	popupPos = {lx: 0, lr: 0, y: 0, w: 117, h: 16}
    }else{
    	popupPos = {lx: 0, lr: 0, y: 0, w: detail_festival[0].offsetWidth, h: detail_festival[0].offsetHeight}
    }
	...
	$(".today").val(month+1)
      ...
      initalToday()
      //不是当前月色块变灰
        for( var i=1; i<=$("#renderMonth .day li").length+1;i++){
			if(!($("#renderMonth .day li:nth-of-type("+i+")").hasClass("cur-month"))){
				if($("#renderMonth .day li:nth-of-type("+i+")").hasClass("cur-day")){
					$("#renderMonth .day li:nth-of-type("+i+")").css("background-color","#fff");
				}else{
					$("#renderMonth .day li:nth-of-type("+i+")").css("background-color","#f8f8f8");
				}
